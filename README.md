### Привет. Это - тренажёр для изучения блокчейна 
...
После применения команд выше, в вашем проекте должны были создаться файлы *Pipfile* и *Pipfile.lock*, в некоторых случаях и *requirements.txt*. Если при выполнении команд выдаётся ошибка, то  сначала проделайте следующий шаг. Если всё равно не будет работать, то загуглите ошибку и почините её, всё гуглится :)
...
* функции `proof_of_work`, `valid_proof`, `valid_chain` и `resolve_conflicts` нужны для достижения консенсуса;
...
```
pipenv run python main.py --port ПОРТ
```
...
Весь тренажёр построен на распределённой сети, которая реализуется через локальные порты твоего компьютера. Представим, что сейчас в цепи только 2 пользователя с разными портами. Для примера, у меня это порт 5000 и 5002
